...
export default function SendPage() {
  ...
  useEffect(() => {
    fetch("/api/admin")
      .then((res) => res.json())
      .then((data: any) => {
        // data가 무엇이든 일단 허용(any)하여 빨간 줄 차단
        if (data?.stickers?.urls) {
          const formatted = data.stickers.urls.map(
            (url: string, i: number) => ({
              id: i,
  ...
  return (
    <div className="flex justify-center min-h-screen bg-white antialiased select-none">
      <div className="w-full max-w-[430px] min-h-screen bg-white flex flex-col relative shadow-2xl">
        ...
        <div className="flex-grow overflow-y-auto p-6 pt-2 pb-32 no-scrollbar">
